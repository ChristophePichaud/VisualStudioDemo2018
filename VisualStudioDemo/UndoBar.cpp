...
{
	return globalUtils.ScaleByDPI(CSize (160, 115));
}
...
int CUndoBar::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_wndList.Create (WS_VISIBLE | WS_CHILD | LBS_NOINTEGRALHEIGHT | LBS_NOTIFY | WS_VSCROLL | LBS_MULTIPLESEL | LBS_OWNERDRAWFIXED | LBS_HASSTRINGS, 
						rectDummy, this, ID_LIST);
	m_wndList.ModifyStyleEx (0, WS_EX_CLIENTEDGE);

	m_wndList.SetFont (pMenuFont);

	if (CBCGPMDIFrameWnd::GetActiveTearOffFrame() != NULL)
	{
		return 0;
	}
...
void CUndoBar::DoUndo ()
{
	...
	CBCGPMDIFrameWnd* pMainFrame = CBCGPMDIFrameWnd::GetActiveTearOffFrame() == NULL ?
		DYNAMIC_DOWNCAST (CBCGPMDIFrameWnd, AfxGetMainWnd()) :
	...
	if (pChildFrame->GetSafeHwnd() != NULL)
	{
		CBCGPVisualStudioGUIDemoView* pView = DYNAMIC_DOWNCAST(CBCGPVisualStudioGUIDemoView, pChildFrame->GetActiveView());
		if (pView->GetSafeHwnd() != NULL)
		{
			pView->DoUndo(pUndoButton);
		}
		else
		{
			ASSERT(FALSE);
