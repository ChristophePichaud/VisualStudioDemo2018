...
void CBCGPVisualStudioGUIDemoView::OnContextMenu(CWnd*, CPoint point)
{
	...
	if (pt.x >= 0 && pt.x < m_pEdit->m_nLeftMarginWidth)
	{
...
void CBCGPVisualStudioGUIDemoView::OnEditIncreaseIndent() 
{
	ASSERT_VALID (m_pEdit);
	VERIFY(m_pEdit->IndentSelection(TRUE));
}

void CBCGPVisualStudioGUIDemoView::OnUpdateEditIncreaseIndent(CCmdUI* pCmdUI) 
{
	ASSERT_VALID (m_pEdit);
	pCmdUI->Enable(m_pEdit->IsIndentEnabled(TRUE));
}

void CBCGPVisualStudioGUIDemoView::OnEditDecreaseIndent() 
{
	ASSERT_VALID (m_pEdit);
	VERIFY(m_pEdit->IndentSelection(FALSE));
}

void CBCGPVisualStudioGUIDemoView::OnUpdateEditDecreaseIndent(CCmdUI* pCmdUI) 
{
	ASSERT_VALID (m_pEdit);
...
{
	ASSERT_VALID (m_pEdit);
	m_pEdit->SetLineNumbersMargin (!m_pEdit->IsLineNumbersMarginVisible (), 30, 10);
}
...
void CBCGPVisualStudioGUIDemoView::OnChangeVisualStyle ()
{
	...
	UINT uiBmpId = theApp.m_b2012Icons ? IDB_IMG_INTELLISENSE_2012 : theApp.m_bHiColorIcons ? IDB_IMG_INTELLISENSE_HC : IDB_IMG_INTELLISENSE;

	CBCGPToolBarImages images;
	images.SetImageSize(CSize(16, 16));
	images.SetTransparentColor(RGB(255, 255, 255));
	...
	images.ExportToImageList(m_imgList);
...
void CBCGPVisualStudioGUIDemoView::OnNcPaint() 
{
...
void CBCGPVisualStudioGUIDemoView::OnEditCompleteWord() 
{
	ASSERT_VALID (m_pEdit);
	m_pEdit->InvokeIntelliSense();
}

void CBCGPVisualStudioGUIDemoView::OnEditViewWhiteSpace()
{
	ASSERT_VALID (m_pEdit);
	m_pEdit->SetViewWhiteSpace (!m_pEdit->IsViewWhiteSpace());
}

void CBCGPVisualStudioGUIDemoView::OnUpdateEditViewWhiteSpace(CCmdUI* pCmdUI)
{
	ASSERT_VALID (m_pEdit);
