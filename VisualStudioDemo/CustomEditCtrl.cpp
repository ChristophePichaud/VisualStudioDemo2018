...
int CCustomEditCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	globalUtils.ScaleByDPI(m_ImageBreak);

	m_ImageCurrent.Load (IDB_CURRENT);
	m_ImageCurrent.SetSingleImage ();
	globalUtils.ScaleByDPI(m_ImageCurrent);
...
BOOL CCustomEditCtrl::OnBeforeInvokeIntelliSense (const CString& strBuffer, int& nCurrOffset, CString& strIntelliSence) const
{
	...
	if (m_bCompleteWord)
	{
		...
		CString strWord;
		if (((CCustomEditCtrl*)this)->GetWordFromOffset(nOffset - 1, strWord) && !strWord.IsEmpty())
		{
			strIntelliSence = strWord;
			nCurrOffset = nOffset - 1;
			return TRUE;
		}
	}
...
{
	CString strIntelliSence = lpszIntelliSense;

	if (lpszIntelliSense == NULL)
	{
		int nOffset = GetCurOffset();
		if (!OnBeforeInvokeIntelliSense(m_strBuffer, nOffset, strIntelliSence))
		{
			return FALSE;
		}
	}

	StringIndex* pStringIndex = NULL;

	if (IsIntelliSenceWord(lpszIntelliSense) && 
		m_mIntelliString.Lookup(lpszIntelliSense, (void*&)pStringIndex))
	{
		CBCGPIntelliSenseData* pData;
		lstIntelliSenseData.RemoveAll();

		if (pStringIndex == NULL)
		{
			return FALSE;
		}
	...
	if (m_bCompleteWord)
	{
		...
		{
			pData = new CBCGPIntelliSenseData;
			pData->m_strItemName.Format(_T("%s - Option %d"), (LPCTSTR)strIntelliSence, i + 1);
			pData->m_nImageListIndex = 0;
			lstIntelliSenseData.AddTail (pData);
		}
